...
Lemma clos_deriv : forall (Γ Δ : @Ensemble (BPropF V)), CompNotDer (Γ, Δ) ->
                                (forall A, wBIC_rules (Γ, A) -> (In _ Γ A)).
Proof.
...
Lemma primality : forall (Γ Δ : @Ensemble (BPropF V)), CompNotDer (Γ, Δ) ->
...
Definition Canon_valuation (P : Canon_worlds) (q : V) : Prop :=
  In _ (fst (proj1_sig P)) (# q).

Lemma C_R_refl : R_refl Canon_relation.
Proof.
unfold R_refl. intros. unfold Canon_relation.
intro. intros. assumption.
Qed.

Lemma C_R_trans : R_trans Canon_relation.
Proof.
unfold R_trans. intros. unfold Canon_relation.
intro. intros. unfold Canon_relation in H0. unfold Canon_relation in H.
apply H0. apply H. auto.
Qed.

Lemma C_R_props : (R_refl Canon_relation * R_trans Canon_relation).
Proof.
split. apply C_R_refl. apply C_R_trans.
Qed.

Lemma C_val_persist : val_persist_R _ Canon_valuation Canon_relation.
Proof.
unfold val_persist_R. intros.
unfold Canon_valuation in H0. unfold Canon_relation in H.
unfold Canon_valuation. apply H. auto.
Qed.

Lemma truth_lemma : forall A (cp : Canon_worlds),
  (wforces Canon_worlds Canon_relation C_R_props Canon_valuation C_val_persist cp A) <->
  (In _ (fst (proj1_sig cp)) A).
Proof.
...
- destruct H.
  ...
  subst. apply Id. apply IdRule_I ; auto.
- apply primality with (Δ:=snd x) in H ; auto. destruct H. left. apply IHA1 ; auto.
...
- destruct x. simpl. destruct (classic (In (BPropF V) e (A1 → A2))).
  ...
  intro. apply gen_wBIC_Deduction_Theorem in H1. apply H0.
  ...
  pose (H s).
  assert (J2: Canon_relation (exist (fun x : Ensemble (BPropF V) * Ensemble (BPropF V) =>
  CompNotDer x) (e, e0) (conj c f)) s). unfold Canon_relation. simpl.
...
- intros. destruct x. simpl in H. destruct v. simpl. destruct x. simpl.
  apply IHA1 in H1. simpl in H1. unfold Canon_relation in H0. simpl in H0.
...
(* Excl A1 A2 *)
- destruct H. destruct H. destruct (classic (In (BPropF V) (fst x) (Excl A1 A2))). auto.
  exfalso. destruct H0. assert (wBIC_rules (fst x, Excl A1 A2) -> False). intro. apply H1.
  apply clos_deriv with (Δ:=snd x) ; auto. apply IHA1 in H0.
  simpl in H0. assert (In (BPropF V) (fst (proj1_sig x0)) (Or A2 (Excl A1 A2))).
  ...
  intros. inversion H4. subst. apply Ax. apply AxRule_I. apply RA11_I. exists A1.
  exists A2. auto. inversion H5. 2: inversion H6. subst. apply Id. apply IdRule_I.
  auto. apply primality with (Δ:=snd (proj1_sig x0)) in H4. destruct H4 ; auto.
  apply H2. apply IHA2. auto. destruct x0. simpl. auto.
- assert (wpair_derrec ((Singleton _ A1), Union _ (snd x) (Singleton _ A2)) -> False).
  ...
  assert (wBIC_rules (Singleton (BPropF V) A1, Or A2 (list_disj (remove eq_dec_form A2 x0)))).
  ...
  pose (wBIC_Deduction_Theorem (Union (BPropF V) (Empty_set (BPropF V)) (Singleton (BPropF V) A1),
  ...
  2: apply MPRule_I. intros. inversion H2. subst.
  pose (wBIC_monot (Empty_set (BPropF V), Excl A1 A2 → list_disj (remove eq_dec_form A2 x0))
  ...
  exists s. unfold Canon_relation. simpl. split. destruct x. simpl.
...
assert (J1: complete (fst (x, x0), snd (x, x0)) /\ (wpair_derrec (fst (x, x0), snd (x, x0)) -> False)). auto.
pose (@exist (prod (Ensemble (BPropF V)) (Ensemble (BPropF V))) CompNotDer (x,x0) J1).
pose (H3 Canon_worlds Canon_relation C_R_props Canon_valuation C_val_persist s).
assert ((forall A : BPropF V, In (BPropF V) Γ A -> wforces Canon_worlds Canon_relation C_R_props
Canon_valuation C_val_persist s A)). intros. apply truth_lemma. auto.
