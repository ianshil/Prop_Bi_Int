...
Proof.
intros. apply sBIC_extens_wBIC. apply wThm_irrel.
Qed.

Lemma simp_Id_gen : forall A Γ, sBIC_rules (Γ, A → A).
Proof.
intros. apply sBIC_extens_wBIC. apply wimp_Id_gen.
Qed.

Lemma scomm_And_obj : forall A B Γ, (sBIC_rules (Γ, And A B → And B A)).
Proof.
intros. apply sBIC_extens_wBIC. apply comm_And_obj.
Qed.

Lemma sContr_Bot : forall A Γ, sBIC_rules (Γ, And A (Neg A) → (Bot V)).
Proof.
intros. apply sBIC_extens_wBIC. apply wContr_Bot.
Qed.

Lemma sEFQ : forall A Γ, sBIC_rules (Γ, (Bot V) → A).
Proof.
intros. apply sBIC_extens_wBIC. apply wEFQ.
Qed.

Lemma smonotR_Or : forall A B Γ, (sBIC_rules (Γ, A → B)) ->
    (forall C, sBIC_rules (Γ, (Or A C) → (Or B C))).
Proof.
intros A B Γ D C.
...
Lemma smonotL_Or : forall A B Γ,
    (sBIC_rules (Γ, A → B)) ->
    (forall C, sBIC_rules (Γ, (Or C A) → (Or C B))).
Proof.
intros A B Γ D C.
...
Lemma scomm_Or_obj : forall A B Γ, (sBIC_rules (Γ, Or A B → Or B A)).
Proof.
intros. apply sBIC_extens_wBIC. apply comm_Or_obj.
Qed.
...
Lemma smonot_Or2 : forall A B Γ,
    (sBIC_rules (Γ, A → B)) ->
    (forall C, sBIC_rules (Γ, (Or A C) → (Or C B))).
Proof.
intros A B Γ D C.
...
Theorem sdual_residuation_gen : forall A B C Γ,
  (sBIC_rules (Γ, (Excl A B) → C) ->
      sBIC_rules (Γ, A → (Or B C))) *
  (sBIC_rules (Γ, A → (Or B C)) ->
      sBIC_rules (Γ, (Excl A B) → C)).
Proof.
...
Theorem sdual_residuation : forall A B C,
  (sBIC_rules (Empty_set _, (Excl A B) → C) ->
      sBIC_rules (Empty_set _, A → (Or B C))) *
  (sBIC_rules (Empty_set _, A → (Or B C)) ->
      sBIC_rules (Empty_set _, (Excl A B) → C)).
Proof.
...
Theorem sBIC_Detachment_Theorem : forall s,
           (sBIC_rules s) ->
           (forall A B Γ, (fst s = Γ) ->
                          (snd s = A → B) ->
                          sBIC_rules (Union _ Γ (Singleton _ (A)), B)).
Proof.
...
* intros A B Γ id1 id2. inversion H1. subst. simpl in id2. subst. simpl.
  ...
  pose (sBIC_comp (Union _ Γ0 (Singleton _ A0), A → B) s Γ0 J3). simpl in s0. assumption.
  apply MPs with (ps:=[(Union _ Γ0 (Singleton _ A), (Imp A B));(Union _ Γ0 (Singleton _ A), A)]).
  2: apply MPRule_I. intros. inversion H3. subst.
  ...
  pose (H _ J01).
  pose (sBIC_monot (Γ0, A0 → A → B) s0 (Union _ Γ0 (Singleton _ A)) J4). assumption.
  inversion H5. subst. pose (H _ J02).
  assert (J4: Included _ (fst (Γ0, A0 → A → B)) (Union _ Γ0 (Singleton _ A))).
  simpl. intro. intro. apply Union_introl. assumption.
  pose (sBIC_monot (Γ0, A0) s0 (Union _ Γ0 (Singleton _ A)) J4). simpl in s1.
...
* intros A B Γ id1 id2. inversion H1. subst. simpl in id2. subst. inversion id2. subst. simpl.
  ...
  pose (sBIC_monot _ J01). apply s. simpl. intro. intros. apply Union_introl. auto. inversion H14.
  inversion H9. inversion H6. subst. apply sBIC_extens_wBIC. apply wTop. inversion H7. inversion H4.
Qed.

Theorem gen_sBIC_Detachment_Theorem : forall A B Γ,
...
assumption.
pose (sBIC_monot (Γ , Bot V) H3 (Union _ Γ (Singleton _ A)) J1). simpl in s. subst. assumption.
inversion H6.
...
pose (sBIC_Detachment_Theorem (Γ, A → B) H3 A B Γ J1 J2). assumption.
...
Lemma sDN_to_form : forall A Γ, (sBIC_rules (Γ, Neg (wNeg A)→ A)).
Proof.
intros A Γ. apply sBIC_extens_wBIC. apply wDN_to_form.
Qed.

Lemma sT_for_DN : forall A n m Γ, (le m n) -> (sBIC_rules (Γ, (DN_form n A) → (DN_form m A))).
Proof.
intros. apply sBIC_extens_wBIC. apply wT_for_DN. assumption.
Qed.

Lemma sExcl_mon : forall A B C,
  (sBIC_rules (Empty_set _, A → B)) ->
  (sBIC_rules (Empty_set _, (Excl A C) → (Excl B C))).
Proof.
...
Lemma smon_Excl : forall A B C,
  (sBIC_rules (Empty_set _, A → B)) ->
  (sBIC_rules (Empty_set _, (Excl C B) → (Excl C A))).
Proof.
intros. apply sdual_residuation.
...
Lemma swNeg_Top : forall A B Γ,
  (sBIC_rules (Γ, ((wNeg A) → B))) ->
  (sBIC_rules (Γ, (Excl (Top _) A) → B)).
Proof.
...
Lemma Top_wNeg_cons : forall A B Γ,
  (sBIC_rules (Γ, A → (Excl (Top _) B))) ->
  (sBIC_rules (Γ, A → (wNeg B))).
Proof.
...
Lemma sOr_imp_assoc : forall A B C D Γ,
  (sBIC_rules (Γ, A → (Or (Or B C) D))) ->
  (sBIC_rules (Γ, A → (Or B (Or C D)))).
Proof.
intros.
...
Lemma sClaim1 : forall A B Γ,
    (sBIC_rules (Γ, (Neg (Excl A B)) → ((wNeg B) → (wNeg A)))).
Proof.
intros A B Γ. apply sBIC_extens_wBIC. apply wClaim1.
Qed.

Lemma sDN_dist_imp : forall A B Γ,
    (sBIC_rules (Γ, (Neg (wNeg (A → B))) → ((Neg (wNeg A)) → (Neg (wNeg B))))).
Proof.
intros A B Γ. apply sBIC_extens_wBIC. apply wDN_dist_imp.
Qed.

Theorem sBIC_Deduction_Theorem : forall s,
           (sBIC_rules s) ->
           (forall A B Γ, (fst s = Union _ Γ (Singleton _ (A))) ->
                          (snd s = B) ->
                          (exists n, sBIC_rules (Γ, (DN_form n A) → B))).
Proof.
intros s D. induction D.
(* Ids *)
...
- intros A B Γ id1 id2. inversion H1. subst. simpl in id1. subst. simpl.
  ...
  sBIC_rules prem)). intros. inversion H4. subst. 2: inversion H5. assumption.
...
Theorem gen_sBIC_Deduction_Theorem : forall A B Γ,
  spair_derrec (Union _ Γ (Singleton _ (A)), Singleton _ (B)) ->
...
- simpl in H1.
  ...
  pose (sBIC_Deduction_Theorem (Union _ Γ (Singleton _ A), Bot V) H1 A (Bot V) Γ J1 J2).
...
- simpl in H1. destruct x. simpl in H1. assert (b=B). pose (H0 b). assert (List.In b [b]). apply in_eq.
  ...
  pose (sBIC_Deduction_Theorem (Union _ Γ (Singleton _ A), B) H1 A B Γ J1 J2). destruct e.
