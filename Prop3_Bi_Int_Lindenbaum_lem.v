...
Lemma der_mult_disj_Bot : forall n Γ A,
  wBIC_rules (Γ, Or A (mult_disj n (Bot V))) -> wBIC_rules (Γ, A).
Proof.
induction n.
...
- simpl. intros.
  ...
  apply wBIC_Deduction_Theorem with (s:=(Union _ Γ (Singleton _ (Or (Bot V) (mult_disj n (Bot V)))), Bot V)) ; auto.
...
(* Pick one (and only one) disjunct in a disjunction. *)

Definition choice_form (Γ Δ : @Ensemble (BPropF V)) (A1 A2 : BPropF V) (* (NotDer : wpair_derrec (Γ, Δ) -> False) *)
...
Definition choice_code (Γ Δ : @Ensemble (BPropF V)) (n :nat) (* (NotDer : wpair_derrec (Γ, Δ) -> False) *) : @Ensemble (BPropF V) :=
...
Lemma Id_list_disj : forall Γ l0 l1,
  wBIC_rules (Γ, list_disj l0 → list_disj (l0 ++ l1)).
Proof.
induction l0 ; intros.
- simpl. apply wEFQ.
...
Lemma list_disj_app : forall l0 Γ A l1,
  wBIC_rules (Γ, A → list_disj (l0 ++ l1)) -> wBIC_rules (Γ, A → (Or (list_disj l0) (list_disj l1))).
Proof.
induction l0.
- simpl. intros.
...
Lemma list_disj_app0 : forall l0 Γ A l1,
  wBIC_rules (Γ, A → (Or (list_disj l0) (list_disj l1))) -> wBIC_rules (Γ, A → list_disj (l0 ++ l1)).
Proof.
induction l0.
- simpl. intros.
...
wBIC_rules (Γ, list_disj (l0 ++ remove_list l0 l1) → Or A (list_disj (l0 ++ remove eq_dec_form A (remove_list l0 l1)))).
Proof.
induction l0.
...
Lemma Id_list_disj_remove : forall Γ l0 l1,
  wBIC_rules (Γ, list_disj l1 → list_disj (l0 ++ remove_list l0 l1)).
Proof.
induction l0.
...
Lemma der_list_disj_remove1 : forall Γ A l0 l1,
    (wBIC_rules (Γ, A → list_disj l0)) ->
    (wBIC_rules (Γ, A → list_disj (l0 ++ remove_list l0 l1))).
Proof.
    ...
    * assert ((fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/ (exists
      ...
      { assert ((fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/
        ...
        { simpl. apply der_list_disj_remove1.
          pose (wBIC_Deduction_Theorem ((Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), list_disj x0))
          H6 b1 (list_disj x0) (nprime_theory Γ Δ n)). apply w ; auto. }
        inversion H9. 2: inversion H10. subst.
        { simpl. apply der_list_disj_remove2.
          pose (wBIC_Deduction_Theorem ((Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b2), list_disj x))
...
Lemma Consist_nprime_theory : forall n (Γ Δ: @Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
  (wBIC_rules (nprime_theory Γ Δ n, Bot V) -> False).
Proof.
pose (strong_induction (fun (n : nat) => forall (Γ Δ : Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
wBIC_rules (nprime_theory Γ Δ n, Bot V) -> False)). apply f. clear f. intros. destruct n.
- apply H0. exists []. repeat split ; auto. apply NoDup_nil. intros. inversion H2.
- simpl in H1. assert (wBIC_rules (nprime_theory Γ Δ n, Bot V)).
  { unfold choice_code in H1. destruct (decoding (S n)).
    + destruct b ; auto. destruct (classic (wpair_derrec (nprime_theory Γ Δ n, Singleton
      (BPropF V) (Or b1 b2)))).
      * assert ((fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/
        (exists DerDisj : wpair_derrec (nprime_theory Γ Δ n, Singleton (BPropF V) (Or b1 b2)),
        In (BPropF V) (choice_form (nprime_theory Γ Δ n) Δ b1 b2 DerDisj) x)) =
        (fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/
        In (BPropF V) (choice_form (nprime_theory Γ Δ n) Δ b1 b2 H2) x)).
        { apply Extensionality_Ensembles. split. intro. intros. inversion H3 ; auto.
          unfold In. auto. destruct H4. unfold In. auto. intro. intros. inversion H3.
          unfold In. auto. unfold In. right. exists H2. auto. }
        rewrite H3 in H1. clear H3. unfold choice_form in H1.
        destruct (classic ((wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Δ) ->
        False))).
        { assert ((fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/
          In (BPropF V) (fun x0 : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x0 \/
          ((wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Δ) ->
          False) -> x0 = b1) /\ (((wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b1), Δ) -> False) -> False) ->  x0 = b2)) x) = 
          (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b1))).
          { apply Extensionality_Ensembles. split. intro. intros. inversion H4.
            unfold In. apply Union_introl. auto. unfold In in H5. destruct H5.
            unfold In. apply Union_introl. auto. destruct H5. unfold In.
            apply Union_intror. unfold In. apply H5 in H3. rewrite H3. apply In_singleton.
            intro. intros. inversion H4. subst. unfold In. auto. subst. inversion H5. subst.
            unfold In. right. right. split. intro. auto. intro. exfalso. apply H6. auto. }
          rewrite H4 in H1. clear H4. exfalso. apply H3. exists []. repeat split.
          apply NoDup_nil. intros. inversion H4. simpl. auto. }
        { assert ((fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/
          In (BPropF V) (fun x0 : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x0 \/
          ((wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Δ) ->
          False) -> x0 = b1) /\ (((wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b1), Δ) -> False) -> False) ->  x0 = b2)) x) =
          (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b2))).
          { apply Extensionality_Ensembles. split. intro. intros. inversion H4.
            unfold In. apply Union_introl. auto. inversion H5. apply Union_introl. auto.
            destruct H6. apply Union_intror. rewrite H7. apply In_singleton. auto. intro.
            intros. inversion H4. subst. unfold In. auto. subst. unfold In. right. right.
            split. intro. exfalso. auto. inversion H5. subst. auto. }
          rewrite H4 in H1. clear H4.
          assert (J4: wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Δ)).
          { destruct (classic (wpair_derrec (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Δ))).
            auto. exfalso. apply H3 ; auto. }
          destruct J4. simpl in H4. destruct H4. destruct H5.
          assert (J1: fst (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b2), Bot V) =
          Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b2)). auto.
          assert (J2: Bot V = Bot V). auto.
          pose (@wBIC_Deduction_Theorem (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b2), Bot V) H1 b2 (Bot V) ((nprime_theory Γ Δ n)) J1 J2).
          assert (wBIC_rules (nprime_theory Γ Δ n, Or b1 b2)).
          { simpl. destruct H2. destruct H2. destruct H7. simpl in H7. simpl in H8.
            destruct x0. simpl in H8.
            apply MP with (ps:=[(nprime_theory Γ Δ n, Imp (Bot V) (Or b1 b2)); (nprime_theory Γ Δ n, Bot V)]).
            2: apply MPRule_I. intros. inversion H9. subst. apply wEFQ. inversion H10. 2: inversion H11.
            subst. auto. inversion H2. subst. assert (J3: List.In b (b :: x0)). apply in_eq. pose (H7 b J3).
            inversion s. subst. destruct x0. simpl in H8. apply absorp_Or1 in H8. auto. exfalso.
            apply H11. assert (List.In b (Or b1 b2 :: b :: x0)). apply in_cons. apply in_eq. pose (H7 b H9).
            inversion s0. subst. apply in_eq. }
          clear H3. clear H. destruct x. simpl in H6.
          assert (J5: fst (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Bot V) =
          Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1)). auto.
          assert (J6: Bot V = Bot V). auto.
          pose (@wBIC_Deduction_Theorem (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b1), Bot V) H6 b1 (Bot V) ((nprime_theory Γ Δ n)) J5 J6).
          apply MP with (ps:=[(nprime_theory Γ Δ n, Imp (Or b1 b2) (Bot V)); (nprime_theory Γ Δ n, (Or b1 b2))]).
          intros. 2: apply MPRule_I. inversion H. subst.
          apply MP with (ps:=[(nprime_theory Γ Δ n, (b2 → Bot V) → (Or b1 b2 → Bot V)); (nprime_theory Γ Δ n, b2 → Bot V)]).
          intros. 2: apply MPRule_I. inversion H3. subst.
          apply MP with (ps:=[(nprime_theory Γ Δ n, (b1 → Bot V) → (b2 → Bot V) → (Or b1 b2 → Bot V)); (nprime_theory Γ Δ n, b1 → Bot V)]).
          intros. 2: apply MPRule_I. inversion H8. subst. apply Ax. apply AxRule_I.
          apply RA4_I. exists b1. exists b2. exists (Bot V). auto. inversion H9. 2: inversion H10.
          subst. auto. inversion H8. 2: inversion H9. subst. auto. inversion H3. 2: inversion H8.
          subst. auto.
          simpl in H6. exfalso. apply Under_nprime_theory with (Γ:=Γ) (Δ:=Δ) (n:=n) ; auto.
          exists (b :: x). repeat split ; auto. simpl.
          apply MP with (ps:=[(nprime_theory Γ Δ n, Imp (Or b1 b2) (Or b (list_disj x))); (nprime_theory Γ Δ n, (Or b1 b2))]).
          intros. 2: apply MPRule_I. inversion H. subst.
          apply MP with (ps:=[(nprime_theory Γ Δ n, (b2 → (Or b (list_disj x))) → (Or b1 b2 → (Or b (list_disj x)))); (nprime_theory Γ Δ n, b2 → (Or b (list_disj x)))]).
          intros. 2: apply MPRule_I. inversion H3. subst.
          apply MP with (ps:=[(nprime_theory Γ Δ n, (b1 → (Or b (list_disj x))) → (b2 → (Or b (list_disj x))) → (Or b1 b2 → (Or b (list_disj x)))); (nprime_theory Γ Δ n, b1 → (Or b (list_disj x)))]).
          intros. 2: apply MPRule_I. inversion H8. subst. apply Ax. apply AxRule_I.
          apply RA4_I. exists b1. exists b2. exists (Or b (list_disj x)). auto. inversion H9. 2: inversion H10.
          subst. assert (J5: fst (Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1), Bot V) =
          Union (BPropF V) (nprime_theory Γ Δ n) (Singleton (BPropF V) b1)). auto.
          assert (J6: (Or b (list_disj x)) = (Or b (list_disj x))). auto.
          pose (@wBIC_Deduction_Theorem (Union (BPropF V) (nprime_theory Γ Δ n)
          (Singleton (BPropF V) b1), (Or b (list_disj x))) H6 b1 (Or b (list_disj x)) ((nprime_theory Γ Δ n)) J5 J6). auto.
          inversion H8. 2: inversion H9. subst.
          apply MP with (ps:=[(nprime_theory Γ Δ n, (Bot V → Or b (list_disj x)) → (b2 → Or b (list_disj x)));
          (nprime_theory Γ Δ n, (Bot V → Or b (list_disj x)))]). 2: apply MPRule_I. intros.
          inversion H9. subst.
          apply MP with (ps:=[(nprime_theory Γ Δ n, (b2 → Bot V) → (Bot V → Or b (list_disj x)) → (b2 → Or b (list_disj x)));
          (nprime_theory Γ Δ n, (b2 → Bot V))]). 2: apply MPRule_I. intros.
          inversion H10. subst. apply Ax. apply AxRule_I. apply RA1_I. exists b2. exists (Bot V).
          exists (Or b (list_disj x)). auto. inversion H11. 2: inversion H12. subst. auto.
          inversion H10. 2: inversion H11. subst. apply wEFQ. inversion H3. subst. auto. inversion H8. }
      * assert (nprime_theory Γ Δ n = fun x : BPropF V => In (BPropF V) (nprime_theory Γ Δ n) x \/
        (exists DerDisj : wpair_derrec (nprime_theory Γ Δ n, Singleton (BPropF V) (Or b1 b2)),
        In (BPropF V) (choice_form (nprime_theory Γ Δ n) Δ b1 b2 DerDisj) x)). apply Extensionality_Ensembles.
        split. intro. intros. unfold In. auto. intro. intros. inversion H3 ; auto. destruct H4.
        exfalso. apply H2. auto. rewrite <- H3 in H1. auto.
    + auto. }
  assert (J1: n < S n). lia. pose (H n J1 Γ Δ H0). auto.
Qed.

Lemma der_nprime_theory_mprime_theory_le : forall n m (Γ Δ: @Ensemble (BPropF V)) A,
  (wBIC_rules (nprime_theory Γ Δ n, A)) -> (le n m) ->
    (wBIC_rules (nprime_theory Γ Δ m, A)).
Proof.
intros.
pose (wBIC_monot (nprime_theory Γ Δ n, A) H (nprime_theory Γ Δ m)).
simpl in w. apply w. intro. intros. apply nprime_theory_extens_le with (n:=n) ; auto.
Qed.

Lemma der_prime_theory_nprime_theory : forall s, (wBIC_rules s) ->
 (forall (Γ Δ: @Ensemble (BPropF V)) A, (prime_theory Γ Δ = fst s) ->
                                        (A = snd s) ->
                                        (exists n, (wBIC_rules (nprime_theory Γ Δ n, A)))).
Proof.
...
Lemma primeness_fst_Lind_pair0 : forall (Γ Δ: @Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
  (forall A1 A2, (wBIC_rules (prime_theory Γ Δ, Or A1 A2)) -> ((In _ (prime_theory Γ Δ) A1) \/ (In _ (prime_theory Γ Δ) A2))).
Proof.
intros.
...
assert (J3: wBIC_rules (nprime_theory Γ Δ x1, Or A1 (Or A2 (mult_disj x0 (Bot V))))).
apply wBIC_monot with (Γ1:=nprime_theory Γ Δ x1) in H1. remember (nprime_theory Γ Δ x1) as C.
...
assert (wBIC_rules (nprime_theory Γ Δ (S x1), A2)).
...
apply wBIC_monot with (Γ1:=nprime_theory Γ Δ x2) in H6. simpl in H6.
...
apply wBIC_Deduction_Theorem with (s:=(Union (BPropF V) (nprime_theory Γ Δ x2) (Singleton (BPropF V) A1), list_disj x4)) ; auto.
apply wBIC_monot with (Γ1:=Union (BPropF V) (nprime_theory Γ Δ x2) (Singleton (BPropF V) A1)) in H11 ; auto ; simpl.
...
apply wBIC_Deduction_Theorem with (s:=(Union (BPropF V) (nprime_theory Γ Δ x2) (Singleton (BPropF V) A2), list_disj x5)) ; auto.
inversion H19. inversion H16. rewrite <- H17.
apply wBIC_monot with (Γ1:=nprime_theory Γ Δ x2) in H1. simpl in H1. auto.
...
Lemma closeder_fst_Lind_pair : forall (Γ Δ: @Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
  (forall A, wBIC_rules (prime_theory Γ Δ, A) -> (In _ (prime_theory Γ Δ) A)).
Proof.
intros. assert (wBIC_rules (prime_theory Γ Δ, (Or A A))).
...
apply primeness_fst_Lind_pair0 in H1. 2: auto. destruct H1 ; auto.
Qed.
...
Lemma Complete_Lind_pair : forall (Γ Δ: @Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
  complete (prime_theory Γ Δ,
  fun x : BPropF V => wBIC_rules (prime_theory Γ Δ, x) -> False).
Proof.
...
Lemma Consist_prime_theory : forall (Γ Δ: @Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
  (wBIC_rules (prime_theory Γ Δ, Bot V) -> False).
Proof.
...
Lemma Under_Lind_pair : forall (Γ Δ: @Ensemble (BPropF V)), (wpair_derrec (Γ, Δ) -> False) ->
  wpair_derrec (prime_theory Γ Δ, fun x : BPropF V => wBIC_rules (prime_theory Γ Δ, x) -> False) ->
   False.
Proof.
...
- assert (wBIC_rules (prime_theory Γ Δ, Bot V)). apply Id.
...
Proof.
intros Γ Δ notder.
exists (prime_theory Γ Δ). exists (fun x => (wBIC_rules ((prime_theory Γ Δ), x) -> False)).
repeat split.
