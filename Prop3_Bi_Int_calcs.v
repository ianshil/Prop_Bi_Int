...
Inductive wBIC_rules (s : ((@Ensemble (BPropF V)) * (BPropF V))) : Prop :=
  | Id : IdRule [] s -> wBIC_rules s
  | Ax : AxRule [] s -> wBIC_rules s
  | MP : forall ps, (forall prem, List.In prem ps -> wBIC_rules prem) -> MPRule ps s -> wBIC_rules s
  | DNw : forall ps, (forall prem, List.In prem ps -> wBIC_rules prem) -> DNwRule ps s -> wBIC_rules s
.

Inductive sBIC_rules (s : ((@Ensemble (BPropF V)) * (BPropF V))) : Prop :=
  | Ids : IdRule [] s -> sBIC_rules s
  | Axs : AxRule [] s -> sBIC_rules s
  | MPs : forall ps, (forall prem, List.In prem ps -> sBIC_rules prem) -> MPRule ps s -> sBIC_rules s
  | DNs : forall ps, (forall prem, List.In prem ps -> sBIC_rules prem) -> DNsRule ps s -> sBIC_rules s
.
