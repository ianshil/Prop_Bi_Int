...
Theorem wBIC_comp : forall s,
          (wBIC_rules s) ->
          (forall Γ,  (forall A, ((fst s) A) -> wBIC_rules (Γ, A)) ->
          wBIC_rules (Γ, (snd s))).
Proof.
intros s D0. induction D0.
(* Id *)
...
Theorem sBIC_comp : forall s,
          (sBIC_rules s) ->
          (forall Γ,  (forall A, ((fst s) A) -> sBIC_rules (Γ, A)) ->
          sBIC_rules (Γ, (snd s))).
Proof.
intros s D0. induction D0.
(* Ids *)
...
Theorem wBIC_struct : forall s,
          (wBIC_rules s) ->
          (forall (f : V -> (BPropF V)),
          (wBIC_rules ((fun y => (exists A, prod ((fst s) A) (y = (subst f A)))), (subst f (snd s))))).
Proof.
intros s D0. induction D0.
(* Id *)
...
Theorem sBIC_struct : forall s,
          (sBIC_rules s) ->
          (forall (f : V -> (BPropF V)),
          (sBIC_rules ((fun y => (exists A, prod ((fst s) A) (y = (subst f A)))), (subst f (snd s))))).
Proof.
intros s D0. induction D0.
(* Ids *)
...
Theorem wBIC_finite : forall s,
          (wBIC_rules s) ->
          (exists (Γ : Ensemble _), prod (Included _ Γ (fst s))
                                     (prod (wBIC_rules (Γ, snd s))
...
- inversion H1. subst. assert (J1: List.In (Γ, A → B) [(Γ, A → B); (Γ, A)]). apply in_eq.
  ...
  apply Union_introl. assumption. pose (@wBIC_monot (x, A → B) w (Union _ x x1) J3). assumption.
  inversion H5. subst. assert (J4: Included _ (fst (x1, A)) (Union _ x x1)). intro. simpl. intro.
  apply Union_intror. assumption. pose (@wBIC_monot (x1, A) w0 (Union _ x x1) J4). assumption.
...
Theorem sBIC_finite : forall s,
          (sBIC_rules s) ->
          (exists (Γ : Ensemble _), prod (Included _ Γ (fst s))
                                     (prod (sBIC_rules (Γ, snd s))
                                     (exists (l : list (BPropF V)), (forall A, ((Γ A) -> List.In A l) * (List.In A l -> (Γ A)))))).
Proof.
intros s D0. induction D0.
(* Ids *)
...
- inversion H1. subst. assert (J1: List.In (Γ, A → B) [(Γ, A → B); (Γ, A)]). apply in_eq.
  ...
  intros. inversion H4. subst.
  assert (J3: Included _ (fst (x, A → B)) (Union _ x x1)). intro. simpl. intro.
  apply Union_introl. assumption. pose (@sBIC_monot (x, A → B) s (Union _ x x1) J3). assumption.
  inversion H5. subst. assert (J4: Included _ (fst (x1, A)) (Union _ x x1)). intro. simpl. intro.
  apply Union_intror. assumption. pose (@sBIC_monot (x1, A) s0 (Union _ x x1) J4). assumption.
  inversion H6. apply MPRule_I.
  exists (x0 ++ x2). intro. split. intro. inversion H4. subst. pose (H2 A0).
  destruct p. apply in_or_app. apply i1 in H5. auto. subst. pose (H3 A0).
  destruct p. apply i1 in H5. apply in_or_app. auto. intro. apply in_app_or in H4.
