...
Lemma wThm_irrel : forall A B Γ, wBIC_rules (Γ, A → (B → A)).
Proof.
intros A B Γ.
...
Lemma wimp_Id_gen : forall A Γ, wBIC_rules (Γ, A → A).
Proof.
intros A Γ.
...
Lemma wContr_Bot : forall A Γ, wBIC_rules (Γ, And A (Neg A) → (Bot V)).
Proof.
intros A Γ.
...
Lemma wmonotR_Or : forall A B Γ,
    (wBIC_rules (Γ, A → B)) ->
    (forall C, wBIC_rules (Γ, (Or A C) → (Or B C))).
Proof.
intros A B Γ D C.
...
Lemma wmonotL_Or : forall A B Γ,
    (wBIC_rules (Γ, A → B)) ->
    (forall C, wBIC_rules (Γ, (Or C A) → (Or C B))).
Proof.
intros A B Γ D C.
...
Lemma comm_Or_obj : forall A B Γ, (wBIC_rules (Γ, Or A B → Or B A)).
Proof.
intros A B Γ.
...
Lemma comm_Or : forall A B Γ, (wBIC_rules (Γ, Or A B)) -> (wBIC_rules (Γ, Or B A)).
Proof.
intros A B Γ D.
...
Lemma wmonot_Or2 : forall A B Γ, (wBIC_rules (Γ, A → B)) ->
    (forall C, wBIC_rules (Γ, (Or A C) → (Or C B))).
Proof.
intros A B Γ D C.
...
Theorem wdual_residuation0 : forall A B C,
  (wBIC_rules (Empty_set _, (Excl A B) → C) ->
      wBIC_rules (Empty_set _, A → (Or B C))).
Proof.
...
Lemma wDN_to_form : forall A Γ, (wBIC_rules (Γ, Neg (wNeg A)→ A)).
Proof.
intros A Γ.
...
Lemma wEFQ : forall A Γ, wBIC_rules (Γ, (Bot V) → A).
Proof.
intros A Γ.
...
Lemma wAThm_irrel : forall A B Γ, wBIC_rules (Γ, (Excl A B) → A).
Proof.
intros A B Γ. pose (wBIC_monot (Empty_set _, Excl A B → A)). apply w.
...
Lemma wTop : forall Γ, wBIC_rules (Γ, Top V).
Proof.
...
Lemma wBiLEM : forall A Γ, wBIC_rules (Γ, Or A (wNeg A)).
Proof.
intros. pose (wBIC_monot (Empty_set _, Or A (wNeg A))). apply w.
...
Theorem wBIC_Detachment_Theorem : forall s,
           (wBIC_rules s) ->
           (forall A B Γ, (fst s = Γ) ->
                          (snd s = A → B) ->
                          wBIC_rules  (Union _ Γ (Singleton _ (A)), B)).
Proof.
...
- intros A B Γ id1 id2. inversion H1. subst. simpl in id2. subst. simpl.
  ...
  pose (wBIC_comp (Union _ Γ0 (Singleton _ A0), A → B) w Γ0 J3). simpl in w0. assumption.
  apply MP with (ps:=[(Union _ Γ0 (Singleton _ A), (Imp A B));(Union _ Γ0 (Singleton _ A), A)]).
  2: apply MPRule_I. intros. inversion H3. subst.
  ...
  pose (wBIC_monot (Γ0, A0 → A → B) w0 (Union _ Γ0 (Singleton _ A)) J4). assumption.
  inversion H5. subst.
  assert (J4: Included _ (fst (Γ0, A0 → A → B)) (Union _ Γ0 (Singleton _ A))).
  simpl. intro. intro. apply Union_introl. assumption.
  pose (wBIC_monot (Γ0, A0)). apply w0. apply H. apply in_cons. apply in_eq.
...
Theorem wBIC_Deduction_Theorem : forall s,
           (wBIC_rules s) ->
...
(* DNw *)
- intros A B Γ id1 id2. inversion H1. subst. simpl in id1. subst. simpl.
  assert (J1: wBIC_rules (Empty_set _, Neg (wNeg A0))).
  ...
  pose (wBIC_monot (Empty_set _, Neg (wNeg A0)) J1 Γ J2). simpl in w.
...
Theorem gen_wBIC_Detachment_Theorem : forall A B Γ,
...
assumption.
pose (wBIC_monot (Γ , Bot V) H3 (Union _ Γ (Singleton _ A)) J1). assumption.
...
pose (wBIC_Detachment_Theorem (Γ, A → B) H3 A B Γ J1 J2). assumption.
...
Theorem gen_wBIC_Deduction_Theorem : forall A B Γ,
  wpair_derrec (Union _ Γ (Singleton _ (A)), Singleton _ (B)) ->
      wpair_derrec (Γ, Singleton _ (A → B)).
Proof.
...
pose (wBIC_Deduction_Theorem (Union _ Γ (Singleton _ A), Bot V) H3 A (Bot V) Γ J1 J2).
...
pose (wBIC_Deduction_Theorem (Union _ Γ (Singleton _ A), B) H3 A B Γ J1 J2). assumption.
...
Theorem wBIC_Dual_Detachment_Theorem : forall A B Δ,
           (wBIC_rules (Singleton _ (Excl A B), list_disj Δ)) ->
           (wBIC_rules (Singleton _ (A), Or B (list_disj Δ))).
Proof.
intros A B Δ D.
...
pose (wBIC_Deduction_Theorem (Singleton _ (Excl A B), list_disj Δ) D (Excl A B) (list_disj Δ)
(Empty_set _) J1 J2).
...
pose (wBIC_Detachment_Theorem (Empty_set _, A → Or B (list_disj Δ)) w A (Or B (list_disj Δ))
...
Theorem wBIC_Dual_Deduction_Theorem : forall A B Δ,
           (wBIC_rules (Singleton _ (A), Or B (list_disj Δ))) ->
           (wBIC_rules (Singleton _ (Excl A B), list_disj Δ)).
Proof.
intros A B Δ D.
...
pose (wBIC_Deduction_Theorem (Singleton _ A, Or B (list_disj Δ)) D A (Or B (list_disj Δ))
(Empty_set _) J1 J2). apply wdual_residuation in w.
assert (J3: @Empty_set (BPropF V) = Empty_set _). reflexivity.
assert (J4: Excl A B → list_disj Δ = Excl A B → list_disj Δ). reflexivity.
pose (wBIC_Detachment_Theorem (Empty_set _, Excl A B → list_disj Δ) w (Excl A B)
...
Lemma remove_disj : forall l B Γ, wBIC_rules (Γ, list_disj l → Or B (list_disj (remove eq_dec_form B l))).
Proof.
induction l.
...
Theorem gen_wBIC_Dual_Detachment_Theorem : forall A B Δ,
...
simpl.
pose (wBIC_Dual_Detachment_Theorem A B x H1).
...
Theorem gen_wBIC_Dual_Deduction_Theorem : forall A B Δ,
  wpair_derrec (Singleton _ (A), Union _ (Singleton _ (B)) Δ) ->
      wpair_derrec (Singleton _ (Excl A B), Δ).
Proof.
intros A B Δ wpair. destruct wpair. destruct H. destruct H0. simpl in H0. simpl in H1.
...
simpl.
pose (wBIC_Dual_Deduction_Theorem A B (remove eq_dec_form B x)). apply w.
...
(* To help for the results about sBIC. *)
...
Lemma wT_for_DN : forall A n m Γ, (le m n) -> (wBIC_rules (Γ, (DN_form n A) → (DN_form m A))).
Proof.
intro A. induction n.
...
Lemma wExcl_mon : forall A B C,
  (wBIC_rules (Empty_set _, A → B)) ->
  (wBIC_rules (Empty_set _, (Excl A C) → (Excl B C))).
Proof.
...
Lemma wmon_Excl : forall A B C,
  (wBIC_rules (Empty_set _, A → B)) ->
  (wBIC_rules (Empty_set _, (Excl C B) → (Excl C A))).
Proof.
intros. apply wdual_residuation.
...
Lemma wOr_Neg : forall A B C Γ,
  (wBIC_rules (Γ, A → (Or B C))) ->
  (wBIC_rules (Γ, (And (Neg B) A) → C)).
Proof.
intros.
...
Lemma wNeg_Top : forall A B Γ,
  (wBIC_rules (Γ, ((wNeg A) → B))) ->
  (wBIC_rules (Γ, (Excl (Top _) A) → B)).
Proof.
...
Lemma Top_wNeg_cons : forall A B Γ,
  (wBIC_rules (Γ, A → (Excl (Top _) B))) ->
  (wBIC_rules (Γ, A → (wNeg B))).
Proof.
...
Lemma Or_imp_assoc : forall A B C D Γ,
  (wBIC_rules (Γ, A → (Or (Or B C) D))) ->
  (wBIC_rules (Γ, A → (Or B (Or C D)))).
Proof.
intros.
...
Lemma wClaim1 : forall A B Γ,
    (wBIC_rules (Γ, (Neg (Excl A B)) → ((wNeg B) → (wNeg A)))).
Proof.
intros A B Γ.
pose (wBIC_monot (Empty_set _, Neg (Excl A B) → wNeg B → wNeg A)). apply w. clear w.
...
Lemma wDN_dist_imp : forall A B Γ,
    (wBIC_rules (Γ, (Neg (wNeg (A → B))) → ((Neg (wNeg A)) → (Neg (wNeg B))))).
Proof.
intros A B Γ.
